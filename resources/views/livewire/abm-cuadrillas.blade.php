<div class="max-w-7xl mx-auto p-6 pt-0 space-y-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-800 dark:text-white">Cuadrillas</h1>
            <p class="text-gray-600 dark:text-gray-300">Administra las cuadrillas y sus áreas</p>
        </div>
        
        <button 
            wire:click="nuevaCuadrilla"
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($cuadrillas as $c)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">{{ $c->area->nombre }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center gap-2">
                                    <button 
                                        wire:click="editarCuadrilla({{ $c->id }})"
                                    ...
                                    <button 
                                        wire:click="confirmarEliminacion({{ $c->id }})"
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center">
                                <div class="text-gray-500 dark:text-gray-400">
                                    <p class="text-lg font-medium">No se encontraron cuadrillas</p>
                                    <p class="text-sm">Intenta ajustar los filtros o crear una nueva cuadrilla.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50 dark:bg-opacity-80">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 w-full max-w-md mx-4">
                ...
                <form wire:submit.prevent="save" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Área *</label>
                        <select wire:model.live="area_id" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-[#77BF43] focus:border-[#77BF43] dark:bg-gray-700 dark:text-white">
                            <option value="">Seleccione un área</option>
                            @foreach($areas as $area)
                                <option value="{{ $area->id }}">{{ $area->nombre }}</option>
                            @endforeach
                        </select>
                        @error('area_id') <div class="mt-2 text-red-600 text-sm">{{ $message }}</div> @enderror
                    </div>
    ...
    @if($showDeleteModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50 dark:bg-opacity-80">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 w-full max-w-md mx-4">
                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Eliminar Cuadrilla</h3>
                <p class="text-sm text-gray-600 dark:text-gray-400">¿Estás seguro de que deseas eliminar la cuadrilla "{{ $selectedCuadrilla?->nombre }}"?</p>
                <div class="flex justify-end space-x-3 mt-6">
                    <button wire:click="cerrarModalEliminacion" type="button" class="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600">Cancelar</button>
                    <button wire:click="eliminar" type="button" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">Eliminar</button>
                </div>
            </div>
        </div>
    @endif
