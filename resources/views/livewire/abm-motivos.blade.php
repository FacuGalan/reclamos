<div class="max-w-7xl mx-auto p-6 pt-0 space-y-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-800 dark:text-white">Gestión de Motivos</h1>
            <p class="text-gray-600 dark:text-gray-300">Administra los motivos de reclamo existentes</p>
        </div>
                        ...
                        ">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center gap-2">
                                        <!-- Editar -->
                                        <button 
                                            wire:click="editarMotivo({{ $motivo->id }})"
                                        ...
                                        <!-- Eliminar -->
                                        <button 
                                            wire:click="confirmarEliminacion({{ $motivo->id }})"
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="text-gray-500 dark:text-gray-400">
                                    ...
                                    <p class="text-lg font-medium">No se encontraron motivos</p>
                                    <p class="text-sm">Intenta ajustar los filtros o crear un nuevo motivo.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 w-full max-w-md mx-4">
                ...
                <div wire:key="form-tipos-{{ $showFormModal ? 'open' : 'closed' }}-{{ $isEditing ? 'edit' : 'create' }}">
                    <form wire:submit.prevent="save" class="space-y-4">
                        ...
                        <!-- Motivo privado -->
                        <div>
                            <label class="flex items-center space-x-2 {{ $puedeEditarPrivada ? 'cursor-pointer' : 'cursor-not-allowed' }}">
                                <input
                                    type="checkbox"
                                    wire:model.live="privada"
                                    @if(!$puedeEditarPrivada) disabled @endif
                                    class="w-4 h-4 text-[#77BF43] bg-gray-100 border-gray-300 rounded focus:ring-[#77BF43] focus:ring-2 dark:bg-gray-700 dark:border-gray-600 {{ $puedeEditarPrivada ? 'cursor-pointer' : 'cursor-not-allowed opacity-60' }}">
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">
                                    Motivo privado (interno)
                                </span>
                            </label>
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                @if($puedeEditarPrivada)
                                    Los motivos privados solo se ven en reclamos internos
                                @else
                                    Se asigna automáticamente según tu tipo de acceso a reclamos
                                @endif
                            </p>
                        </div>
                ...
                <div class="flex items-start mb-4">
                    ...
                    <div class="ml-3">
                        ...
                        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                            ¿Estás seguro de que deseas eliminar el motivo "{{ $selectedMotivo?->nombre }}"? Esta acción no se puede deshacer.
                        </p>
                        @if($selectedMotivo)
